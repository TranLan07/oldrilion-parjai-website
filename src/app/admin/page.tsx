...
export default function AdminPage() {
  ...
  const apiMap: Record<Tab, string> = {
    users: "/api/admin/users", recruitment: "/api/admin/recruitment", channels: "/api/admin/channels",
    missions: "/api/admin/missions", pages: "/api/admin/pages",
    lore: "/api/admin/lore", rules: "/api/admin/rules", grades: "/api/admin/grades", specs: "/api/admin/specializations", dictionary: "/api/admin/dictionary",
  };
  ...
  const load = useCallback(async () => {
    ...
    if (tab === "users" || tab === "channels") {
      const [gr, sp, us] = await Promise.all([
        ...
        tab === "channels" ? fetch("/api/admin/users") : Promise.resolve(null),
      ]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      ...
      {tab === "lore" && <ContentTab sections={loreSections} endpoint="/api/admin/lore" label="Lore" api={api} />}
      {tab === "rules" && <ContentTab sections={ruleSections} endpoint="/api/admin/rules" label="Règle" api={api} />}
          ...
          {editing?.id === u.id ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/users", "PUT", editing); setEditing(null); }} className={btnGreen}>Sauvegarder</button>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/users", "DELETE", { id: u.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
...
function RecruitmentTab({ recruitments, load }: { recruitments: Recruitment[]; load: () => void }) {
  async function handle(id: string, action: "approve" | "reject") {
    const res = await fetch("/api/admin/recruitment", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action }) });
    const data = await res.json();
...
function GradesTab({ grades, api }: { grades: Grade[]; api: (e: string, m: string, b?: object) => Promise<void> }) {
  ...
  async function create() {
    if (!form.name) return;
    await api("/api/admin/grades", "POST", form);
        ...
        <div key={g.id} className="flex items-center justify-between rounded-lg border border-accent-dim/20 bg-surface p-4">
          {deleting === g.id ? (
            <div className="flex flex-1 items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/grades", "DELETE", { id: g.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
                <button onClick={() => setDeleting(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <input type="number" value={g.order} onChange={(e) => api("/api/admin/grades", "PUT", { id: g.id, order: parseInt(e.target.value) })} className={`w-16 ${inp}`} />
                <label className="text-xs text-foreground/50">Perm:</label>
                <select value={g.defaultPermission} onChange={(e) => api("/api/admin/grades", "PUT", { id: g.id, defaultPermission: parseInt(e.target.value) })} className={`w-16 ${inp}`}>
                  {Array.from({ length: 11 }, (_, i) => <option key={i} value={i}>{i}</option>)}
                </select>
...
function ChannelsTab({ channels, users, grades, specs, api, load }: {
  ...
  async function addMember(channelId: string) {
    if (!addUserId) return;
    await fetch("/api/admin/channels", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: channelId, action: "addMember", userId: addUserId }) });
    setAddUserId("");
    load();
  }

  async function removeMember(channelId: string, userId: string) {
    await fetch("/api/admin/channels", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: channelId, action: "removeMember", userId }) });
    load();
  }
        ...
        <div key={c.id} className="rounded-sm p-4" style={{ background: "var(--beskar-800)", border: `1px solid ${c.isPrivate ? "rgba(201,168,76,0.3)" : "var(--beskar-600)"}` }}>
          {deleting === c.id ? (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/channels", "DELETE", { id: c.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
                <button onClick={() => setDeleting(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : (
...
function MissionsTab({ missions, api }: { missions: Mission[]; api: (e: string, m: string, b?: object) => Promise<void> }) {
  ...
  async function create() {
    ...
    await api("/api/admin/missions", "POST", form);
        ...
        <div key={m.id} className="rounded-lg border border-accent-dim/20 bg-surface p-4">
          {deleting === m.id ? (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/missions", "DELETE", { id: m.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
                <button onClick={() => setDeleting(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <select value={m.status} onChange={(e) => api("/api/admin/missions", "PUT", { id: m.id, status: e.target.value })} className={`${inp} w-auto`}>
...
function PagesTab({ pages, api }: { pages: PagePerm[]; api: (e: string, m: string, b?: object) => Promise<void> }) {
  ...
  async function create() {
    ...
    await api("/api/admin/pages", "POST", form);
        ...
        <div key={p.id} className="flex items-center justify-between rounded-lg border border-accent-dim/20 bg-surface p-4">
          {deleting === p.id ? (
            <div className="flex flex-1 items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/pages", "DELETE", { id: p.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
                <button onClick={() => setDeleting(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={p.minPermission} onChange={(e) => api("/api/admin/pages", "PUT", { id: p.id, minPermission: parseInt(e.target.value) })} className={`w-20 ${inp}`}>
...
function SpecsTab({ specs, api }: { specs: Spec[]; api: (e: string, m: string, b?: object) => Promise<void> }) {
  ...
  async function create() {
    if (!form.name) return;
    await api("/api/admin/specializations", "POST", form);
        ...
        <div key={s.id} className="rounded-sm border p-4" style={{ borderColor: s.secret ? "rgba(162,89,224,0.3)" : "var(--beskar-600)", background: "var(--beskar-800)" }}>
          {editing?.id === s.id ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/specializations", "PUT", editing); setEditing(null); }} className={btnGreen}>Sauvegarder</button>
                <button onClick={() => setEditing(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : deleting === s.id ? (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={async () => { await api("/api/admin/specializations", "DELETE", { id: s.id }); setDeleting(null); }} className={btnDanger}>Confirmer</button>
                <button onClick={() => setDeleting(null)} className={btnSecondary}>Annuler</button>
              </div>
            </div>
          ) : (
