...
export async function POST(req: NextRequest) {
  ...
  if (existing && !force) {
    return NextResponse.json({
      conflict: true,
      existing: existing,
      message: `Le mot "${key}" existe déjà avec la traduction "${existing.mandoa}".`,
    }, { status: 409 });
  }
  ...
  if (existing) {
    const updated = await prisma.dictionaryEntry.update({
      where: { french: key },
      data: { mandoa: mandoa.trim() },
    });
    return NextResponse.json(updated);
  }

  const entry = await prisma.dictionaryEntry.create({
    data: { french: key, mandoa: mandoa.trim() },
  });
...
export async function PUT(req: NextRequest) {
  ...
  const entry = await prisma.dictionaryEntry.update({
    where: { id },
    data: {
      ...(french && { french: french.trim().toLowerCase() }),
      ...(mandoa && { mandoa: mandoa.trim() }),
    },
  });
  return NextResponse.json(entry);
}
